...
$(function(){
    window.addEventListener("message", function(event){
        ...
        if (event.data.newalert) {
            var toInsert = `<tr id="table-container" class="table-container"><td class="table-code">Alert</td><td class="content-table-text">${event.data.content}</td><td class="table-id">ID:` + `${event.data.id}</td></tr>`
        ...
        if (event.data.inConfig == true) {
            ...
            const htmlString = `<h1 class="cust-text" id="cust-text">Press I to finish the customization</h1>`
        }
